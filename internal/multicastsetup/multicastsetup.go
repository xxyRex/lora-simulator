/*
 * multicastsetup.go - LoRaWAN远程多播设置协议实现文件
 *
 * 该文件实现了LoRaWAN远程多播设置协议（Remote Multicast Setup v1.0.0），
 * 用于FUOTA过程中的多播组配置。主要功能包括：
 * - 定义多播组相关的命令标识符（CID）
 * - 实现多播组的创建、状态查询和删除等命令
 * - 提供Class B和Class C多播会话的设置功能
 * - 处理设备对多播命令的响应
 *
 * 该协议在FUOTA过程中负责创建和管理多播组，使得同一固件可以同时发送给多个设备，
 * 从而提高固件分发的效率。
 */
